...
public sealed class ValidateAccountDirectoryRequestAttributeTests
{
    [Fact]
    public void OnActionExecuting_trims_valid_query()
    {
        var request = new AccountDirectoryRequest { Query = "  admin  " };
        using var services = CreateServices();
        var context = CreateContext(request, services);

        new ValidateAccountDirectoryRequestAttribute().OnActionExecuting(context);

        Assert.Equal("admin", request.Query);
    ...
    {
        var request = new AccountDirectoryRequest
        {
    ...
    private static ActionExecutingContext CreateContext(
        AccountDirectoryRequest request,
